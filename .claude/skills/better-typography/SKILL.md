...
<!--
Vendored from jakubkrehel/skills, skills/better-typography/.
Pinned to commit 6c43b20ce00f720eed854591abe480127629c191 (2026-08-20).
The six reference .md files sit beside this one and came from the same commit.
Upstream text is verbatim; Compass notes are at the bottom of this file.
-->
...
- **No Tailwind utilities.** 74 of 74 components style with inline `style={{}}`
  objects; `src/index.css` holds only tokens and a few globals. Write fixes as
  CSS properties in the inline object or in `index.css`. `css-cheat-sheet.md`
  maps declarations to Tailwind classes — read it in the other direction.
- **Numbers are the product.** Times, degrees, orbs, and percentages are on
  nearly every surface. `font-variant-numeric: tabular-nums` is applied at 24
  sites already; a new one that stacks or aligns numbers needs it too.
- **The symbol faces are load-bearing and have burned us.** Glyphs render from
  "Noto Sans Symbols 2" / "Noto Sans Symbols". A vendored subset once shipped
  with no planet glyphs in its cmap. Check a font's coverage before swapping
  or subsetting one.
- **Sibling skills named upstream are not installed.** `better-writing`,
  `better-accessibility`, `better-colors`, and `better-layout` are referenced
  in the text but absent here. For copy, use `no-ai-slop` (required by
  CLAUDE.md). For accessibility, use `fixing-accessibility`.
