...
| ☐ | **The promised AI 503 guard does not exist.** `isOpenAiConfigured` is exported and its own comment says "every AI route must check [it] … see routes that already do this for the pattern" — **zero routes do** (grepped 2026-07-31). With the boot crash now fixed, an unconfigured key no longer kills the server, but the eight routes importing `openai` will fail with whatever the SDK throws against a placeholder credential instead of a contained 503. Cheap: one guard clause per route, or one middleware. |
