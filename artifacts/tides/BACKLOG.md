...
| ◐ | **Google Calendar correctness** | ☑ Today's bounds now send real RFC 3339 instants. ☑ OAuth `state` is HMAC-signed and expires in 10min (was unsigned base64 — anyone could forge which profile a Google account attached to). ☑ `postMessage` targets our own origin and the listener verifies `e.origin` (was `'*'`). ☐ Still open: `/status` should verify refreshability rather than a row's existence; a visible Reconnect state. | P0 |
