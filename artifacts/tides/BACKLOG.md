...
| ◐ | **Regression suite + CI landed** | ☑ 47 tests, one per bug that ACTUALLY SHIPPED, each verified to fail when its bug is reintroduced; GH Actions runs them under America/Chicago, Asia/Kolkata and UTC; **Railway now refuses to deploy if they fail**. ☐ Still wanted: the Playwright path, integration tests for write paths, and the root `pnpm typecheck` is red on legacy libs (health-tracker only — both Compass apps typecheck clean in their own builds). | P1 |
