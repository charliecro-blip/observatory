...
| ☑ | ~~**Set `ENGINE_TOKENS` on Railway**~~ | **DONE — confirmed live 2026-08-12** by probing production: `/api/engine/activities` without a token returns `401 "invalid engine token"`, not the 503 that means unset. The engine namespace is enabled; remaining step (if any) is on AstroLyrica's side — confirm it holds a matching token and is making calls. | ENGINE-API-SPEC |
...
☑ **A second UTC day-rollover, 2026-07-31** — `GET /planning/windows?date=` applied **UTC** day bounds to a `timestamptz`. Measured against a real server+DB in America/Chicago: **4 of 7 windows filed on the wrong day** — everything from 19:00 local onward fell out of "today" and yesterday's evening climbed in (5h for Austin, 5.5h the other way for Kolkata). The app-wide sweep below missed it because that hunted `toISOString().slice(0,10)` on date STRINGS. It bit hardest in the evening — the shutdown ritual, evening re-homing and the cascade all read exactly the band that vanished. Now the client sends the day's boundaries as instants (`localDayRange`) and the server filters between them, half-open. After: 0 of 7.
...
☑ **OpenAI client no longer crashes the whole server** at boot when unconfigured — ⚠️ **this was only two-thirds true until 2026-07-31.** The fix landed in `src/client.ts`, but `src/index.ts` re-exports `./image`, and `image/client.ts` + `audio/client.ts` still threw at module load. Every route importing the package root went through that barrel, so the server printed *"AI routes will 503 instead of crashing the server"* and then crashed on the very next line — found by trying to boot it locally without AI keys. Both siblings now mirror the pattern. **A guard is only as good as the sibling modules its barrel drags in.**
...
☑ **The block-done verb was locked from the inside** (2026-07-31) — `BlockCheck`, the only way to mark a scheduled block complete, sat INSIDE the evening card's `didAnything` branch, which is true only if you already kept a habit, closed a task, or completed a block. On a day whose only activity was blocks it never rendered: you could record a block only if you had already recorded something else. The door shipped the session before was unreachable for exactly the people who plan in blocks — and it is one of the three signals the done-pattern reads.
...
| ☑ | **`testerId` as a bearer token** | ☑ The query-param fallback is gone and `/account/sync` no longer re-issues the recovery code. ☑ Per-device session tokens landed 2026-08-16: the gate on `/api` refuses a claimed account that cannot present the token that claimed it. ☑ **And the rollout window now ends** (2026-08-19). Trust-on-first-use shipped with no expiry, which made it a hole rather than a rollout — an account claims on its OWN next boot, so an account nobody opens never claims and stays readable and writable by anyone holding its id. Measured against production three days in: **1 of 20 profiles claimed, and none of the other 19 had ever had a row in `account_sessions`** — the gate was protecting one person, and five of the nineteen carry hand-typed ids (`orrery-demo`, `felt-test`, `cadence-test`, `obs_ns3`, `obs_push_route_verify`) that are typed rather than guessed. `lib/tofuWindow.ts` sets the deadline (default **2026-08-23T00:00Z**, `COMPASS_TOFU_DEADLINE` overrides it on Railway without a deploy); past it the gate refuses unclaimed profiles and `/account/claim` 410s. The recovery code stays the way back in, verified end to end against a scratch server. | **done** |
...
| ◐ | **Regression suite + CI landed** | ☑ 77 tests, one per bug that ACTUALLY SHIPPED, each verified to fail when its bug is reintroduced; GH Actions runs them under America/Chicago, Asia/Kolkata and UTC; **Railway now refuses to deploy if they fail**. ☑ The first **integration** test (`tests/deletion.integration.test.ts`, real DB, real deletes) — skipped unless `TEST_DATABASE_URL` is set, a deliberately *different* variable from `DATABASE_URL` so it can never inherit production. ☑ **The typecheck now actually checks (2026-07-31).** api-server's project references pointed at `dist/` declarations nothing built — 16 TS6305 errors, and cross-package imports **were not being validated at all**. Proof: an import of a symbol its package never exported passed typecheck *and* build, then threw `ReferenceError` per request. Dropped the references (the packages' `exports` point at source anyway); 16 masked errors became 23 real ones, all fixed — including **p-retry's `AbortError`**, a named export in v7, so `pRetry.AbortError` was `undefined` and every non-rate-limit batch failure threw "not a constructor" instead of aborting. Also added the missing `@types/node` that made `tsc --build` fail at the root and short-circuit the whole script. **Now: api-server 0, typecheck:libs 0, tides 0.** ☐ The root script finally runs to completion and shows **health-tracker's own 21 pre-existing errors** (natal.tsx ×12, cultivator.tsx ×4, track/planning ×2) — never a regression, just no longer hidden; separate work in a legacy app.
☐ **Voice messages never record what the user said.** `event.type === "user_transcript"` compared types with no overlap — `voiceChatStream` only yields `"transcript"` (the *assistant's* words) and `"audio"`, so `userTranscript` was always empty and every voice message stores `"[voice message]"`. Dead branch removed; the real fix is running `speechToText` over the uploaded buffer, which costs an extra API call per message. health-tracker only.
☑ **Integration coverage for the scheduling write paths (2026-08-05)** — `tests/writes.integration.test.ts`, same `TEST_DATABASE_URL` discipline as the deletion test (a deliberately different variable, so it can never inherit production). Six tests over a real table: the two columns added this week persist, the window link stays null when it should, reads are tester-scoped, an update touches one account only, and every field the scheduler reads round-trips. **Proved by breaking it**: dropping `activity_key` fails two of the six; restoring it passes. This is the gap that let the activity question ship read-only — a pure test cannot see a missing column. ☐ Still wanted: the Playwright path. | P1 |
...
| ◐ | ☑ **The journal's "will retry" now does** (2026-07-31) — it was a plain falsehood: the flag was set, no timer armed, and only further typing ever retried. `lib/outbox.ts` adds debounce, backoff (5s·15s·60s·5m), retry on `online`, adoption of unsent text left on disk by a previous session, and a manual Retry; plus the four honest statuses. Newer text supersedes an in-flight send (so a slow success for older text can't bury a later edit under "saved"), and exhausted attempts keep the words rather than dropping them. Verified by typing with the API stopped and restarting it — the text arrived unattended. ☑ **Planner drafts survive a refresh** (2026-07-31) — `rawList`/`cards`/`result`/`dropped` were component state, so a reload threw away a dump, its AI parse, and a woven schedule. Persisted per tester, cleared on commit. The rule worth knowing: the **list** comes back, the **schedule** doesn't — a weave names future moments, so once they've passed it describes a sky that has moved on, and restoring it would present stale timing as current (§10). Judged by the *earliest* block, not the first in the array. Extracted to `lib/plannerDraft.ts` and tested against a supplied clock. ☐ Still open: **Ask history is in-memory**. |
| ☑ | **GuidingStarsHub mutations skip `r.ok`** — **done 2026-07-31, and this row was the accurate one.** §2 claimed all seven were fixed; an audit of all 18 mutations in the file found three still silent, including the two named here. `toggleHabitToday` checked neither branch, `logSession` was silent, and `addStep`'s unchecked project-create parsed an **error body as a project** — `proj.id` undefined, the step then created against no project at all, reported as success. (A fourth flagged by the audit script, `createLinked`, was a false positive: its two `fetch(` occurrences are the arms of one ternary and it does check.) |
| ☑ | **The promised AI 503 guard** — **done 2026-07-31, and my own framing of it above was wrong.** The flag was unreachable: exported from `./client` but never from the package barrel, so routes importing the package root genuinely could not see it. And "503 everywhere" was the wrong instruction — three routes (`associate`, `chart/explicate`, `planning/breakdown`) already fall back to a **deterministic** answer, so refusing would have removed three working features. Final rule: never call the model with a placeholder credential, prefer the deterministic answer where one exists, 503 only for `blueprint/generate` (nothing to degrade to), leave the two streaming routes alone (they already contain their error). ⚠️ **Also**: re-exporting the flag from a helper under its own name made esbuild emit a self-referential binding — `ReferenceError` on every request, through a clean build *and* a clean typecheck. The typecheck also missed the absent barrel export, because api-server's 16 TS6305 project-reference errors mean **cross-package imports are not actually being validated**. A red baseline hides real errors — see §2's typecheck row. |
...
| ☑ | **Planner review: move, not just drop** (Sunsama/Akiflow) | 1–1.5d | **done** — `Planner.tsx:352`; the runner-up windows the weaver already computed are now offered, and a move is reversible ("Keep it here"). *Was still marked open on 2026-07-31 despite having shipped.* |
...
| ☑ | **Deliberate re-homing of undone work** in the evening ritual (Sunsama shutdown) | 1.5–2d | **done 2026-07-31.** `POST /planning/rehome/suggest` + a "→ move it" affordance on each past-due open block. Not "same time tomorrow" — tomorrow is scored by the same grader the weaver used, so the offer suits the WORK. Now genuinely uses `PATCH /planning/windows/:id`, which had never been called by any client. Appears only once a block's hour has gone (offering earlier = the app deciding you won't do it), and picking a time is the only verb — doing nothing leaves it alone. Slot logic extracted to `lib/rehome.ts` so the invariants are DB-free testable: no past times, no collisions, end fits inside bedtime, empty list rather than a bad 3am slot. **Caught by the extraction**: options could overlap each other (a 90-min block offered at 9:30 *and* 10:30) — spacing is now the block's own length. |
| ☑ | **Live NL parse preview in Quick Capture** (Todoist/Fantastical) | 1–2d | **done 2026-07-31** — `lib/parseWhen.ts`, deterministic (no AI, no network, re-parsed on every keystroke). Preview lists only the lines a date was found in, showing the title *as it will be saved*, because the surprise to prevent is words silently leaving the title. **Rejection affordance**: tap the chip to keep the words as text (keyed by line TEXT, not index — indices shift as you type above a line and would move a rejection onto someone else's task), plus Fantastical's `"quoting"` escape. **Dates only, never times** — `due_date` has no time column, so "2pm" stays visible in the title rather than being rendered back at a precision the row can't hold (§10). Numeric `8/7` deliberately unparsed: ambiguous worldwide, and a wrong date that looks right is worse than none. 25 tests, expectations computed at run time from all seven weekdays; mutation-checked. |
| ☐ | **Graceful degradation: compress · skip-today · log-past-work** (Reclaim) | 1d | `POST /planning/windows/:id/complete` exists, never called. |
| ☑ | ★ **Pre-seed two dailies on first run** (Structured) | ~4h | **done** — `habits.ts:213`, anchored to real sunrise/sunset and idempotent. Verified live on a fresh account 2026-07-31. *Was still marked open despite having shipped.* |
| ☑ | **Consent-based cascade** — open lane | 2–3d | **done 2026-07-31.** `POST /planning/cascade/preview` + `/apply`, card in the evening ritual beside BlockCheck. It always asks, and it always names the cost **per block, before you agree** — graded in the weaver's own `Tier`/`TIER_NOTE`, moved into `lib/timingTier.ts` and shared so there is no second vocabulary. Preview writes nothing; apply takes the instants the user was *shown* (no recompute) in one transaction. Options: shift all · just the next one · leave them — and "just the next one" warns in visible text when it would run into the block after. **Threshold measured, not chosen**: the median put half of every day at "against"; raw values are incomparable across elements (water's best hour < air's p25); energy is normalised within each element's own daily range at 0.10, which marks 6–18% of a day across three unlike skies. |
