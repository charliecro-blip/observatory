...
| ◐ | **Google Calendar correctness** | ☑ Today's bounds now send real RFC 3339 instants (it sent a bare local string; Google returns HTTP 200 + empty list, indistinguishable from "no events"). ☐ Still open: sign+expire OAuth `state`, validate the `postMessage` origin (currently `*`), `/status` should verify refreshability, visible Reconnect state. | P0 |
| ◐ | **Write-status + query-key bugs** | ☑ Today invalidated `["tasks"]` while its visible query is keyed `["tasks-today", …]` — not a prefix, so the refresh silently missed and the UI waited on a 30s poll; now invalidates both. ☐ Still open: Today's task-toggle `r.ok`, advisor memory-save, GCal disconnect, the GuidingStars set. | P0 |
...
| ◐ | **Regression suite + CI landed** | ☑ `pnpm test` — 32 pure tests, one per bug that ACTUALLY SHIPPED (local-day vs UTC incl. DST, longitude-as-timezone with the real +2h Austin error, 0°/180° aspect perfection, push()-returns-a-number, jsonArray guard, query-key prefix matching, habit cadence, task rollover COALESCE, email subject variation, article agreement, open-redirect). Each was verified to FAIL when its bug is reintroduced. GitHub Actions runs it under America/Chicago, Asia/Kolkata and UTC, and `pnpm build` now refuses to ship if it fails. ☐ Still wanted: the Playwright path (onboarding → task/habit → reflection → next local day) and integration tests for the write paths. | P0 |
