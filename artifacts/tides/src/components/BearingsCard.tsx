    ...
    <div style={{ background: "var(--color-card)", border: "1px solid var(--color-border)", borderRadius: 12, padding: "14px 18px 12px", marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 9, alignItems: "baseline", marginBottom: 6 }}>
        ...
        <div style={{ fontSize: 12.5, color: "var(--color-foreground)", lineHeight: 1.55 }}>
          ...
          <span style={{ color: "var(--color-muted)" }}> · <span aria-hidden="true" style={{ fontFamily: "var(--font-symbol)" }}>{PLANET_GLYPH[fix.year.lord] ?? ""}</span> {fix.year.lord} holds the year</span>
          {nextHit && (
        ...
        <div style={{ display: "flex", gap: 9, alignItems: "baseline", marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--color-border)" }}>
          ...
          <div style={{ fontSize: 12, color: "var(--color-foreground)", lineHeight: 1.7, minWidth: 0 }}>
            {(currents!.transitsByHouse as any[]).map((t: any, i: number) => {
              ...
              const line = (
                <>
                  <span aria-hidden="true" style={{ fontFamily: "var(--font-symbol)" }}>{PLANET_GLYPH[t.planet] ?? ""}</span> {t.planet} through your {ord(t.house)}
        ...
        <div style={{ display: "flex", gap: 9, alignItems: "baseline", marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--color-border)" }}>
          ...
          <div style={{ fontSize: 12, color: "var(--color-foreground)", lineHeight: 1.7, minWidth: 0 }}>
            {(currents!.majorTransits as any[])
              ...
              <div key={i}>
                <span aria-hidden="true" style={{ fontFamily: "var(--font-symbol)" }}>{PLANET_GLYPH[t.transitPlanet] ?? ""}</span> {t.transitPlanet} {String(t.aspect).toLowerCase()} your {t.natalPlanet}
        ...
        <div style={{ display: "flex", gap: 9, alignItems: "baseline", marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--color-border)" }}>
          ...
          <div style={{ fontSize: 12, color: "var(--color-foreground)", lineHeight: 1.7, minWidth: 0 }}>
            {(currents!.cautionWindows as any[]).slice(0, 3).map((w: any, i: number) => (
              <div key={i}>
                <span aria-hidden="true" style={{ fontFamily: "var(--font-symbol)" }}>{PLANET_GLYPH[w.transitPlanet] ?? ""}</span> {w.transitPlanet} {String(w.aspect ?? "").toLowerCase()} your {w.natalPlanet}
