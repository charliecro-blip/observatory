      ...
      {active.map(({ c, diff, orbDeg }, i) => {
        ...
        return (
          <div key={`${c.planet}-${c.angle}-${i}`} style={{
            ...
            <span className="phrase-in" style={{ fontSize: 16, flexShrink: 0 }}><span aria-hidden="true" style={{ fontFamily: "var(--font-symbol)" }}>{PLANET_GLYPH[c.planet] ?? "⚡"}</span></span>
            ...
            <div style={{ fontSize: 10.5, background: `${col}22`, color: col, padding: "2px 7px", borderRadius: 4, fontWeight: 700, flexShrink: 0 }}>
              ● {rising ? "↑" : ""} {c.angle}
            </div>
          </div>
        );
